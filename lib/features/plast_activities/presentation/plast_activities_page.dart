...
class PlastActivitiesPage extends StatefulWidget {
  const PlastActivitiesPage({super.key});

  @override
  State<PlastActivitiesPage> createState() => _PlastActivitiesPageState();
}

class _PlastActivitiesPageState extends State<PlastActivitiesPage> {
  List<PlastActivityModel> _activities = [];
  ...
  Future<void> _loadActivities() async {
    ...
    final activities = (response as List)
        .map((e) => PlastActivityModel.fromJson(e))
        .toList();
  ...
  Future<bool> _confirmDelete(PlastActivityModel activity, AppLocalizations l10n) async {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                    ...
                    return Dismissible(
                      ...
                      child: Card(
                        child: ListTile(
                          ...
                          title: Text(activity.projectName),
                          subtitle: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              if (activity.position.isNotEmpty)
                                Text('${l10n.position}: ${activity.position}'),
                              if (activity.area.isNotEmpty)
                                Text('${l10n.area}: ${activity.area}'),
                              if (activity.date != null)
                                Text(DateFormat('dd.MM.yyyy').format(activity.date!)),
                            ],
                          ),
                          isThreeLine: activity.position.isNotEmpty && activity.area.isNotEmpty,
                          onTap: () async {
                            await context.push('/plast-activities/edit', extra: activity);
                            _loadActivities();
                          },
                        ),
                      ),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await context.push('/plast-activities/add');
          _loadActivities();
        },
