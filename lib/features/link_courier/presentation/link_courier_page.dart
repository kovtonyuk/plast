...
class LinkCourierPage extends StatefulWidget {
  const LinkCourierPage({super.key});

  @override
  State<LinkCourierPage> createState() => _LinkCourierPageState();
}

class _LinkCourierPageState extends State<LinkCourierPage> {
  List<LinkCourierModel> _units = [];
  ...
  Future<void> _loadUnits() async {
    ...
    final response = await Supabase.instance.client
        ...
        .select()
        .eq('user_id', userId)
        .order('first_steps_date', ascending: false);

    final units = (response as List)
        .map((e) => LinkCourierModel.fromJson(e))
        .toList();
  ...
  Future<bool> _confirmDelete(LinkCourierModel unit, AppLocalizations l10n) async {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                      ...
                      child: Card(
                        child: ListTile(
                          leading: CircleAvatar(
                            ...
                            child:
                                const Icon(Icons.connect_without_contact, color: Colors.white),
                          ),
                          title: Text(unit.name),
                          subtitle: unit.firstStepsDate != null
                              ? Text(
                                  '${l10n.firstStepsDate}: ${DateFormat('dd.MM.yyyy').format(unit.firstStepsDate!)}')
                              : null,
                          onTap: () async {
                            await context.push('/link-courier/edit', extra: unit);
                            _loadUnits();
                          },
                        ),
                      ),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await context.push('/link-courier/add');
          _loadUnits();
        },
