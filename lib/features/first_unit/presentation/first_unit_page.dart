...
class FirstUnitPage extends StatefulWidget {
  const FirstUnitPage({super.key});

  @override
  State<FirstUnitPage> createState() => _FirstUnitPageState();
}

class _FirstUnitPageState extends State<FirstUnitPage> {
  List<FirstUnitModel> _units = [];
  ...
  Future<void> _loadUnits() async {
    ...
    final units = (response as List)
        .map((e) => FirstUnitModel.fromJson(e))
        .toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                  ...
                  child: Card(
                    child: ListTile(
                      ...
                      subtitle: Column(
                        ...
                        children: [
                          if (unit.firstStepsDate != null)
                            Text(
                              '${l10n.firstStepsDate}: ${DateFormat('dd.MM.yyyy').format(unit.firstStepsDate!)}',
                            ),
                          if (unit.scarfTyingDate != null)
                            Text(
                              '${l10n.scarfTyingDate}: ${DateFormat('dd.MM.yyyy').format(unit.scarfTyingDate!)}',
                            ),
                        ],
                      ),
                      onTap: () async {
                        await context.push('/first-unit/edit', extra: unit);
                        _loadUnits();
                      },
                    ),
                  ),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await context.push('/first-unit/add');
          _loadUnits();
        },
